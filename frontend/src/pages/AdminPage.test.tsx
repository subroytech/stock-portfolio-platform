...
describe('AdminPage', () => {
  test('defaults to the My API(s) tab, with the Manage User Attribute tab and a Back to Home link visible', async () => {
    mockAdminSession();
    renderPage();
    ...
    expect(screen.getByRole('button', { name: 'Manage User Attribute' })).toBeInTheDocument();
    // The 4 grouped sub-tabs aren't shown until Manage User Attribute is active.
    expect(screen.queryByRole('button', { name: 'Manage Users' })).not.toBeInTheDocument();
  });

  test('clicking Manage User Attribute reveals its 4 sub-tabs, defaulting to Manage Users', async () => {
    mockAdminSession();
    renderPage();
    await screen.findByText('FMP (Financial Modeling Prep)');

    await userEvent.click(screen.getByRole('button', { name: 'Manage User Attribute' }));
  ...
  test('switching tabs renders the corresponding panel', async () => {
    ...
    // Switching sub-tabs never requires re-clicking the parent tab.
  ...
  test('an admin-ish session missing api_keys:manage_own does not see the "My API(s)" tab at all', async () => {
    ...
    renderPage();
    await screen.findByRole('button', { name: 'Manage User Attribute' }); // wait for session to resolve
  ...
  test('a session without usage_audit:view does not see the "User Usage" tab', async () => {
    mockAdminSession(); // default helper - no usage_audit:view
    renderPage();
    await screen.findByText('FMP (Financial Modeling Prep)');
    expect(screen.queryByRole('button', { name: 'User Usage' })).not.toBeInTheDocument();
  });
});
