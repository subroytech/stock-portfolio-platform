...
import { useLongTermAnalysis } from '../api/longTermAnalysis';
      ...
      <main className="flex flex-col gap-6 p-4 sm:p-6">
        ...
        {data && (
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-card bg-bg-card p-4 shadow-card">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <span className={`rounded-btn px-3 py-1 text-sm font-semibold ${RATING_STYLES[data.mediumTerm.rating]}`}>
                    MT: {data.mediumTerm.rating}
                  </span>
                  <span className={`rounded-btn px-3 py-1 text-sm font-semibold ${RATING_STYLES[data.longTerm.rating]}`}>
                    LT: {data.longTerm.rating}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div>
                  ...
                  {data.consensus.totalAnalysts > 0 ? (
                    ...
                    <p className="mt-3 text-sm text-text-muted">Analyst consensus data not available.</p>
                  )}
                </div>
                  ...
                  <tbody className="text-text-secondary">
                    ...
                    {data.valuation.peerCount > 0 && (
                      <tr className="border-t border-border italic text-text-muted">
                        <td className="py-2">Peer-Group Average (approximation, not a true sector index)</td>
            ...
            {/* MT/LT conviction */}
            <div className="rounded-card bg-bg-card p-4 shadow-card">
              <h2 className="mb-3 text-sm font-semibold text-text-primary">MT / LT Conviction Rating</h2>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className={`rounded-card p-4 ${RATING_STYLES[data.mediumTerm.rating]}`}>
                  <p className="text-xs uppercase tracking-wide text-text-muted">Medium Term — 12 to 18 Months</p>
                  <p className="mt-1 text-lg font-bold">{data.mediumTerm.rating} <span className="text-sm font-normal">({data.mediumTerm.score}/6)</span></p>
                  <p className="mt-2 text-sm text-text-secondary">{data.mediumTerm.rationale}</p>
                </div>
                <div className={`rounded-card p-4 ${RATING_STYLES[data.longTerm.rating]}`}>
                  <p className="text-xs uppercase tracking-wide text-text-muted">Long Term — 3 Years and Beyond</p>
                  <p className="mt-1 text-lg font-bold">{data.longTerm.rating} <span className="text-sm font-normal">({data.longTerm.score}/7)</span></p>
                  <p className="mt-2 text-sm text-text-secondary">{data.longTerm.rationale}</p>
                </div>
              </div>
              <p className="mt-4 rounded-card bg-bg-primary p-3 text-xs text-text-muted">
                ⚠️ Conviction ratings are algorithmically derived from live data (analyst consensus, earnings growth,
                revenue trend, forward valuation). They are not financial advice. Qualitative factors — management
                quality, competitive moat, macro outlook — are not captured here. Always do your own due diligence.
              </p>
            </div>
