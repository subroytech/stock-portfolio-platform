...
describe('useLogout', () => {
  ...
  // deferred clear()), and clear() must be deferred past the current render/commit (a
  // setTimeout(0), flushed here via fake timers) - not run clear() first/synchronously. That
  // older ordering left any still-mounted query observer (session's own, or e.g. Dashboard's
  // /portfolios) with no cached data ("needs fetching") for one moment, which immediately
  // re-fired a real request - 401ing since the server had just cleared the auth cookie.
  test('session is null immediately; other cached queries are cleared only after a deferred tick', async () => {
    ...
    // Immediately after mutateAsync resolves (before the deferred tick): session already
    // null, but portfolios hasn't been cleared yet - proving the order isn't "clear everything
    // at once."
    expect(queryClient.getQueryData(['session'])).toBeNull();
    expect(queryClient.getQueryData(['portfolios'])).toEqual({ portfolios: [{ id: '1' }] });

    await act(async () => {
      await new Promise((resolve) => setTimeout(resolve, 10)); // let the deferred setTimeout(0) fire
    });

    expect(queryClient.getQueryData(['session'])).toBeNull();
    expect(queryClient.getQueryState(['session'])).toBeDefined(); // present (null), not removed
    expect(queryClient.getQueryData(['portfolios'])).toBeUndefined(); // cleared after the deferred tick
  });
});
